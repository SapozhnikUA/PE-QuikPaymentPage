...
З адміністративного режиму (`?admin`, після входу за паролем) доступна кнопка **«Створити рахунок (PDF)»**, поля **«Платник»** і **«Примітка»** та два спадні переліки: **«Останні рахунки (30 днів)»** та **«Останні посилання (30)»**.

Рахунок формується на основі поточних значень полів `Сума`, `Призначення` та (необов'язково) `Платник`, а також реквізитів ФОП із `config.php`. Дата рахунку — поточна (часовий пояс береться з `timezone`), номер — наступний після найбільшого номера, вже записаного у реєстрі.

PDF містить (за наявності) рядок «Платник», реквізити отримувача, IBAN, банк, МФО, ЄДРПОУ банку, таблицю з призначенням і сумою, суму прописом, невеликий платіжний QR-код і місце для підпису. Кириличний шрифт вбудовується безпосередньо у файл — LibreOffice, wkhtmltopdf чи інші зовнішні залежності не потрібні.

Перед друком у PDF призначення доповнюється посиланням на сам рахунок: `<призначення>, згідно рахунку № <номер> від <дата>`. Цей доповнений текст — саме той, що показаний і в рядку «Призначення платежу» у блоці реквізитів, і в таблиці, і закодований у QR, і збережений у реєстрі: скрізь один і той самий рядок. Додавання суфікса ідемпотентне: якщо призначення вже закінчується таким самим «, згідно рахунку № N від ДД.ММ.РРРР» (саме це відбувається при повторному завантаженні рахунку з реєстру), він не додається вдруге.
...
**Платник** — необов'язкове поле (наприклад, «ФОП Іванов І.І.» чи назва компанії-замовника). На відміну від примітки, воно потрапляє в сам PDF — окремим рядком «Платник» одразу над «Одержувач» у блоці реквізитів, — але не в QR (стандарт QR НБУ не передбачає окремого поля платника). Якщо поле лишити порожнім, рядок у PDF просто не з'являється. Під час набору поле пропонує підказки з платників, які вже зустрічалися в реєстрі раніше (за тим самим принципом, що й підказки шаблонів для «Призначення» — див. нижче), незалежно від того, за які останні 30 днів чи раніше вони з'явились.

**Примітка** — необов'язкове поле суто для внутрішнього використання (номер договору, ім'я клієнта тощо). У сам PDF і в QR вона не потрапляє, тільки в реєстр.

Кожен згенерований рахунок додає рядок до `invoices.csv.php`:

```text
Дата генерації; Номер рахунку; Дата рахунку; Сума; Призначення; Примітка; Платник
```

Якщо на сервері вже є `invoices.csv.php` від старішої версії — без колонки «Примітка» (5 колонок) чи без колонки «Платник» (6 колонок), — перший же новий рахунок автоматично «доростить» заголовок до поточних 7 колонок, не чіпаючи вже наявні рядки.

**Спадний перелік рахунків** (замінює собою окрему кнопку завантаження реєстру) показує рахунки за останні 30 днів у форматі «№ · призначення — сума». Вибір будь-якого пункту повторно завантажує саме той PDF (за допомогою `action=invoice_get`, за номером рахунку) — це той самий файл, що видавався під час створення, а не його наближена копія. Останній пункт списку, **«Завантажити реєстр (CSV)»**, завантажує весь `invoices.csv.php` одним файлом (з BOM — коректно відкривається в Excel).

**Спадний перелік посилань**, поруч із кнопками «Повне посилання» / «Анулювати ключ», показує останні 30 коротких посилань із `links.json` у тому самому форматі «призначення — сума». Вибір пункту копіює в буфер обміну готове коротке посилання (`?код`) для повторного використання. Останній пункт, **«Завантажити links.json»**, — це попередня кнопка завантаження реєстру посилань, вона нікуди не поділась, просто стала останнім пунктом списку.

Ендпоінти `action=invoice` та `action=invoice_get` доступні лише з дійсним `X-Key`. `action=invoice` не «витрачає» номер рахунку, якщо PDF не вдалося згенерувати; `action=invoice_get` не створює нового запису в реєстрі — лише відтворює вже наявний за номером (разом з платником, якщо він був заданий).

`action=invoices` також повертає `payers` — список унікальних значень поля «Платник» з усього реєстру (не лише за останні 30 днів), найновіші перші; саме на ньому й будуються підказки під час набору.
...
- [ ] рядок з примітками з'являється у `invoices.csv.php` (призначення — з додатком «згідно рахунку…», примітка та платник — окремими колонками);
- [ ] заданий «Платник» з'являється в PDF окремим рядком над «Одержувач», а без нього рядок не показується;
- [ ] під час набору «Платника» пропонуються підказки з раніше введених значень (з усього реєстру, не лише за 30 днів);
- [ ] спадний перелік «Останні рахунки (30 днів)» показує щойно створений рахунок і дозволяє завантажити його повторно;
- [ ] останній пункт цього переліку завантажує реєстр як CSV;
- [ ] спадний перелік «Останні посилання (30)» показує щойно скопійоване посилання і копіює його повторно при виборі;
- [ ] останній пункт цього переліку завантажує `links.json`.
