...
З адміністративного режиму (`?admin`, після входу за паролем) доступна кнопка **«Створити рахунок (PDF)»**, поле **«Примітка»** для внутрішніх позначок і два спадні переліки: **«Останні рахунки (30 днів)»** та **«Останні посилання (30)»**.
...
```text
Дата генерації; Номер рахунку; Дата рахунку; Сума; Призначення; Примітка
```

Якщо на сервері вже є `invoices.csv.php` від версії без колонки «Примітка» (5 колонок), перший же новий рахунок автоматично «доростить» заголовок до 6 колонок, не чіпаючи вже наявні рядки.
...
Ендпоінти `action=invoice` та `action=invoice_get` доступні лише з дійсним `X-Key`. `action=invoice` не «витрачає» номер рахунку, якщо PDF не вдалося згенерувати; `action=invoice_get` не створює нового запису в реєстрі — лише відтворює вже наявний за номером.
...
- [ ] рядок з примітками з'являється у `invoices.csv.php` (призначення — з додатком «згідно рахунку…», примітка — окремою колонкою);
